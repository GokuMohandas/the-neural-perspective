...
tf.flags.DEFINE_string("filter_sizes", [3, 4, 5],
    "filter sizes for each of the cnn filters")

tf.app.flags.DEFINE_float("num_filters", 128,
...
tf.app.flags.DEFINE_integer("num_hidden_units", 128,
        ...
        for epoch_num, epoch in enumerate(generate_epoch(train_X, train_y,
            ...
            for batch_num, (batch_X, batch_y, batch_seq_lens) in enumerate(epoch):
                ...
                _, loss, accuracy = model.step(sess, batch_X, batch_seq_lens,
                    batch_y, dropout_keep_prob=FLAGS.dropout_keep_prob,
                    forward_only=False, sampling=False)
            ...
            for valid_epoch_num, valid_epoch in enumerate(
                ...
                for valid_batch_num, \
                    ...
                    loss, accuracy = model.step(sess,
                        ...
                        dropout_keep_prob=1.0, forward_only=True,
                        sampling=False)
...
def sample():
    ...
    with tf.Session() as sess:
        ...
        probability = model.step(sess, batch_X=test_sentence,
