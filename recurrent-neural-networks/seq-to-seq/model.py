...
import data_utils
from my_seq2seq_models import (
	embedding_attention_seq2seq_with_biRNN_encoder,
	)


def rnn_cell(rnn_unit, num_hidden_units, dropout, num_layers):
	...
	# Single cell
	single_cell = rnn_cell_type(num_hidden_units)
	...
	if num_layers > 1:
		stacked_cell = tf.nn.rnn_cell.MultiRNNCell([single_cell] * num_layers)

	return stacked_cell

class s2s_model(object):
	...
	def __init__(self,
		...
		cell = rnn_cell(rnn_unit, size, dropout, num_layers)

		# The seq2seq function: we use embedding for the input and attention.
		def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
			return embedding_attention_seq2seq_with_biRNN_encoder(
					encoder_inputs,
					decoder_inputs,
					cell,
		...
		# Our targets are decoder inputs shifted by one, we do len(decoder_inputs)-1 b/c of <EOS>.
